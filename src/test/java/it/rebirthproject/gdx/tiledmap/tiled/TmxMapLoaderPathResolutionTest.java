...
package it.rebirthproject.gdx.tiledmap.tiled;
 ...
 *
 * <p>
 * Verifies that per-tile image paths declared inside a .tsx file are resolved
 * relative to the .tsx file, not the .tmx map file (regression for GitHub
 * #7818).
 */
public class TmxMapLoaderPathResolutionTest {
    ...
    // --- helpers ---
    /**
     * ImageResolver that records every path it is queried for. Returns null
     * textures (callers must ensure addStaticTiledMapTile is a no-op).
     */
    ...
    /**
     * TmxMapLoader that skips actual tile construction so null TextureRegions
     * are safe, and skips property loading that requires Gdx.app.
     */
    ...
    /**
     * TmjMapLoader that skips actual tile construction so null TextureRegions
     * are safe, skips property loading that requires Gdx.app, and avoids calls
     * to Gdx.app.log().
     */
    ...
    public void externalTsxTileImagesResolvedRelativeToTsxFile() throws Exception {
        ...
        File mapsDir = new File(tempDir, "maps");
        mapsDir.mkdirs();
        File tsDir = new File(tempDir, "ts");
        tsDir.mkdirs();
        ...
        assertEquals(1, resolver.requestedPaths.size(), "Expected exactly one tile image to be resolved");

        String resolvedPath = resolver.requestedPaths.get(0).replace(File.separatorChar, '/');
        String expectedSuffix = "ts/img/tile.png";

        assertTrue(resolvedPath.endsWith(expectedSuffix),
                "Image path should be resolved relative to the .tsx file (ts/img/tile.png), but was: " + resolvedPath);

        // explicitly verify it was NOT resolved relative to the .tmx file
        assertFalse(resolvedPath.endsWith("maps/img/tile.png"),
                "Image path must NOT be resolved relative to the .tmx file (maps/img/tile.png)");
    }

    /**
     * Embedded tileset (no external .tsx) — image path stays relative to the
     * .tmx file. This ensures the fallback path is not broken by the fix.
     */
    @Test
    public void embeddedTilesetImagesResolvedRelativeToTmxFile() throws Exception {
        File mapsDir = new File(tempDir, "maps2");
        mapsDir.mkdirs();
        ...
        assertTrue(resolvedPath.endsWith(expectedSuffix),
                "Embedded tileset image should be resolved relative to the .tmx file, but was: " + resolvedPath);
    }

    /**
     * External TSJ with collection-of-images tiles in a sub-directory (JSON
     * variant).
     *
     * <pre>
     *   maps/
     ...
     * </pre>
     *
     * Verifies that addStaticTiles resolves images relative to the .tsj file,
     * not the .tmj map file.
     */
    @Test
    public void externalTsjTileImagesResolvedRelativeToTsjFile() throws Exception {
        ...
        assertTrue(resolvedPath.endsWith(expectedSuffix),
                "Image path should be resolved relative to the .tsj file (ts/img/tile.png), but was: " + resolvedPath);

        // explicitly verify it was NOT resolved relative to the .tmj file
        assertFalse(resolvedPath.endsWith("maps/img/tile.png"),
                "Image path must NOT be resolved relative to the .tmj file (maps/img/tile.png)");
    }

    /**
     * Embedded tileset in TMJ (no external .tsj source) — image path stays
     * relative to the .tmj file (JSON variant). This ensures the fallback path
     * is not broken by the fix.
     */
    @Test
    public void embeddedTilesetInTmjMap() throws Exception {
        File mapsDir = new File(tempDir, "maps3");
        mapsDir.mkdirs();
        File mapsImgDir = new File(mapsDir, "img");
        mapsImgDir.mkdirs();
        ...
        assertTrue(resolvedPath.endsWith(expectedSuffix),
                "Embedded tileset image should be resolved relative to the .tmj file, but was: " + resolvedPath);
    }
}
